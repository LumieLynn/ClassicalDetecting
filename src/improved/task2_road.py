...
输出路径: output/improved/task2_road/
"""
...
JUMP_LAT_ABS_MIN = 15.0    # lateral 跳变绝对下限（像素）
JUMP_ARC_ABS_MIN = 30.0    # arc 跳变绝对下限（像素）

# ======================== 中心线抖动抑制 ========================
RD_JITTER_THRESH = 8.0     # 平均偏差 > 此值 → 判定抖动，加重旧权重
RD_LOSS_THRESH = 25.0      # 平均偏差 > 此值 → 检测失败，完全沿用旧中心线
RD_JITTER_EMA = 0.85       # 抖动时旧中心线权重
RD_NORMAL_EMA = 0.5        # 正常时新旧权重
...
class RoadPathDetector:
    ...
    def __init__(self, img_w, img_h):
        ...
        self._prev_pts = None      # 上一帧中心线，用于 EMA 平滑
        self._ema = 0.5            # 平滑系数（0=不用旧值, 1=不用新值）
    ...
    def detect(self, gray):
        ...
        if self._prev_pts is not None and len(self._prev_pts) == n:
            ...
            if mean_diff > RD_LOSS_THRESH:
                # 检测严重失败，沿用旧中心线
                return False
            wt = (RD_NORMAL_EMA if mean_diff <= RD_JITTER_THRESH
                  else RD_JITTER_EMA)
            smooth = []
            for (px, py), (qx, qy) in zip(pts, self._prev_pts):
                smooth.append((wt * qx + (1 - wt) * px,
                               wt * qy + (1 - wt) * py))
            pts = smooth
...
def extract_template_from_ref(ref_path):
    ...
    for cnt in contours:
        ...
        cx = M['m10'] / M['m00']
        cy = M['m01'] / M['m00']
        dist = np.sqrt((cx - img_center[0] + TM_POS_BIAS_X * w) ** 2
                       + (cy - img_center[1]) ** 2)
    ...
    out_dir = "output/improved/task2_road"
...
def main():
    ...
    for i in range(anchor_idx - 1, -1, -1):
        ...
        x1 = max(0, cx - bw // 2 - REV_PAD)
        y1 = max(0, cy - bh // 2 - REV_PAD)
        x2 = min(width, cx + bw // 2 + REV_PAD)
        y2 = min(height, cy + bh // 2 + REV_PAD)
        search = gray[y1:y2, x1:x2]

        best_score = -1.0
        best_match = None
        for sf in REV_SCALES:
            s = curr_scale * sf
            sw = int(base_w * s)
            sh = int(base_h * s)
            if (sw < REV_MIN_SIZE or sh < REV_MIN_SIZE
        ...
        if best_score > REV_MIN_SCORE:
            ...
            history[i] = (best_match[0], best_match[1],
    ...
    frame_idx = 0
    frames_written = 0
    prev_ncc = best_anchor_score
    prev_cx, prev_cy = cx, cy
    last_road_detect = anchor_idx
    ...
    while True:
        ...
        pad = max(FWD_PAD, int(max(curr_w, curr_h) * (1.8 if in_occlusion else 1.2)))
        ...
        fg_roi = fg_prob[y1:y2, x1:x2] if fg_prob is not None else None

        # --- NCC 搜索 ---
        if prev_ncc > FWD_NCC_GOOD:
            fg_alpha = FG_ALPHA_HEALTHY
        elif prev_ncc > FWD_NCC_MARGINAL:
            fg_alpha = FG_ALPHA_REDUCED
        else:
            fg_alpha = 0.0

        best_ncc = -1.0
        best_match = None
        for sf in FWD_SCALES:
            s = curr_scale * sf
            sw = int(base_w * s)
            sh = int(base_h * s)
            if (sw < FWD_MIN_SIZE or sh < FWD_MIN_SIZE
                    or sw > search.shape[1] or sh > search.shape[0]):
                continue
            scaled = cv2.resize(curr_tmpl, (sw, sh))
            score, (lx, ly) = foreground_weighted_ncc(
                search, scaled, fg_roi, alpha=fg_alpha)
            if score > best_ncc:
                best_ncc = score
                best_match = (lx + x1, ly + y1, sw, sh, s)
        ...
        if road_motion.initialized and road_detector.model.ready:
            ...
            if ncc_rp is not None:
                ...
                # 跳变检测：NCC 很高时跳过（坐标突变 = 道路模型漂移，非跟踪错误）
                if best_ncc < 0.60:
        ...
        track_healthy = (best_ncc > FWD_NCC_GOOD
                         or (best_ncc > FWD_NCC_MARGINAL and road_ok))
        if track_healthy and (road_ok or not road_motion.initialized):
            ...
            if road_jump:
                ...
                # 不更新 road_motion — 跳变坐标不可靠
                if consecutive_lost > FWD_LOST_MAX:
                    is_tracking = False
                    in_occlusion = False
            else:
                # NCC 高但道路不一致（非跳变）→ NCC 优先，重置道路模型
                in_occlusion = False
                consecutive_lost = 0
                final_cx, final_cy = x + w // 2, y + h // 2
                kalman.update(final_cx, final_cy)
                ...
                if ncc_rp is not None:
                    road_motion.reset_at(ncc_rp[0], ncc_rp[1])
                    road_traj.append(ncc_rp)
        ...
        if (time_to_update or scale_dropped) and not in_occlusion \
        ...
        if frame_idx % PROG_INTERVAL == 0:
            ...
            print(f"  帧 {frame_idx}: NCC={best_ncc:.2f} s={curr_scale:.2f}"
                  f"{ocstr}{jstr}{rstr}")
...
def _save_road_trajectory_plot(road_traj, output_dir):
    ...
    x_min -= mx; x_max += mx; z_min -= mz; z_max += mz
